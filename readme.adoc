...
Using a batch size of 10,000, the median benchmark time for updating in random order was 6.1 hours, and the median benchmark time for updating in sorted primary key order was 50.6 minutes. Full results are in https://docs.google.com/spreadsheets/d/1rojlWUPbvqDWQ4Wu_x1oPGRQtRn_KYsRjpWUXOZlK1o/edit#gid=0[this spreadsheet] under the heading _updating 1,000,000 rows via iodku, varying iodku batch size_. *This is approximately an 8x speedup compared to Method 2 and a 60x speedup compared with Method 1.*
